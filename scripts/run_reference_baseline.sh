...
#
# 2026-08-29時点の既知の状態(docs/steps/step_01_reference_baseline.md 参照):
#   このホストには cmake / make / gcc / g++ が存在せず、非対話的 sudo も
#   使えないため、前提3(acadosのビルド)を満たせていない。
#   そのため下記のpreflightチェックは意図的に失敗し、原因を明示して
#   終了する(場当たり的に実行を続けて不可解なエラーを出さないため)。
# ============================================================================
...
echo "=== preflight OK. Quadruped-PyMPC 公式サンプルを起動します ==="
