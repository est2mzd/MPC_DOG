...
LOG_DIR="${REPO_ROOT}/artifacts/logs/quadsdk_step01"
mkdir -p "${LOG_DIR}"

ROBOT_NS="robot_1"
FORWARD_VEL_MPS="${FORWARD_VEL_MPS:-0.3}"          # 前進速度指令[m/s]
DURATION_S="${DURATION_S:-10.0}"                   # cmd_velを送り続ける実時間[s]
...
CAMERA_DISTANCE_M="${CAMERA_DISTANCE_M:-8.72}"     # 固定カメラのlookat点からの距離[m]。
