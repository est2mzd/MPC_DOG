"""Step 03: 前進方向に 2 m 間隔・幅 5 m・長さ 30 cm の穴(トレンチ)を並べた
平面マップで、穴に落ちずに前進できるかを記録するハーネス。

マップ: `src/trial/assets/scene_gaps.xml`(生成: `src/trial/assets/gen_scene_gaps.py <depth>`)
  - y ∈ [-2.5, 2.5] の 5 m 幅の通路。x 方向に 1.7 m の凸条(box, 上面 z=0)を並べ、
    間に 0.30 m 幅・深さ 5 cm のトレンチ(= 穴)を 2.0 m 間隔で作る。
    トレンチ底は連続スラブ(z = -0.05)なので「底なしの穴」ではなく
    「5 cm 深・30 cm 長・5 m 幅の轍(わだち)」。
  - 穴の中心は x = 1, 3, 5, ...(凸条の中心は x = 0, 2, 4, ...)。
  - ロボットは y=0(穴の横中央)を +x 方向に歩く。
  - 底なしの深い穴(depth 0.5 m)は blind の Quadruped-PyMPC では最初の穴で
    転落した(足場回避をしないため)。浅い轍にすると轍を踏み越えて前進できる。
実行時に gym_quadruped の robot_model/scene_gaps.xml へこの XML をコピーし、
`QuadrupedEnv(scene="gaps", ...)` で読み込ませる(external/ は変更しない)。
...
環境変数: STEP03_VEL / STEP03_FREQ / STEP03_SECONDS で上書き可。
実行: `bash scripts/trial/run_step_03.sh`
"""
...
# ============================================================================
NUM_SECONDS = float(os.environ.get("STEP03_SECONDS", "20"))     # 記録するシミュレーション実時間[s]
INITIAL_FORWARD_VEL_MPS = float(os.environ.get("STEP03_VEL", "0.3"))   # 前進速度指令[m/s]
GAIT_STEP_FREQ_HZ = float(os.environ.get("STEP03_FREQ", "2.0"))  # 歩容の周波数[Hz]。trot既定1.4を上書きする

GAP_SPACING_M = 2.0     # 穴の中心間隔[m](マップ生成と一致させること)
GAP_LENGTH_M = 0.30     # 穴の x 方向長さ[m]
FIRST_GAP_X_M = 1.0     # 最初の穴の中心 x 座標[m]
...
SCENE_SRC = REPO_ROOT / "src" / "trial" / "assets" / "scene_gaps.xml"
LOG_DIR = REPO_ROOT / "artifacts" / "logs" / "step_03"
GIF_DIR = REPO_ROOT / "artifacts" / "gifs"
SUMMARY_CSV_PATH = LOG_DIR / "trials_summary.csv"


def _stage_scene() -> None:
    """src/trial/assets/scene_gaps.xml を gym_quadruped の robot_model/ へコピーする。
    ...
    """
    dst = Path(gym_quadruped.__file__).parent / "robot_model" / "scene_gaps.xml"
...
def main() -> None:
    ...
    gif_path = GIF_DIR / f"step_03_{trial_id_str}.gif"
    ...
    print(
        f"Step 03: gap crossing  |  v={INITIAL_FORWARD_VEL_MPS} m/s  "
    ...
    for step in range(n_steps):
        ...
        if step % frame_stride == 0:
            ...
            overlay = (
                f"Step 03: gap crossing (unmodified Quadruped-PyMPC, blind)\n"
