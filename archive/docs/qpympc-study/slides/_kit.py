...
class Deck:
    ...
    def notes(self, slide, text):
        """Bottom ※ gloss. Structure of the page does not change."""
        if isinstance(text, (list, tuple)):
            parts = [("※ " + t.lstrip("※ ").lstrip("※")) for t in text if t]
            text = "    ".join(parts)
        elif text and not text.lstrip().startswith("※"):
            text = "※ " + text
        rounded(slide, Inches(0.35), Inches(6.50), Inches(12.6), Inches(0.68), SOFT, line=LINE)
        add_text_box(slide, Inches(0.45), Inches(6.52), Inches(12.4), Inches(0.64), text, size=11, color=MUTED, anchor=MSO_ANCHOR.MIDDLE)
    ...
    def title_slide(self, title: str, subtitle: str, meta: str, series: str = "Quadruped-PyMPC  /  速度指令から12トルクまで"):
        slide = self.blank()
        rect(slide, 0, 0, Inches(0.22), H, TEAL)
        rect(slide, 0, 0, W, Inches(0.18), NAVY)
        add_text_box(slide, Inches(0.7), Inches(1.7), Inches(12), Inches(0.4), series, size=16, color=TEAL, bold=True)
    ...
    def bullets(self, slide, items, left=0.5, top=1.85, width=12.3, height=4.5, size=17):
    ...
    def eq_lesson(self, kicker: str, title: str, latex: str, name: str, background: str, intent: str, variables: list, fontsize: float = 20, note: str | None = None):
        """One equation per slide: 背景, 意図, 式, 変数表, ※注釈."""
        ...
        add_text_box(slide, Inches(0.4), Inches(3.68), Inches(12.5), Inches(0.24), "変数説明  （記号を覚えなくてよい。この表を見る）", size=12, color=TEAL, bold=True)
        n = len(variables)
        self.table(slide, variables, 0.4, 3.92, 12.5, min(2.45, 0.40 * n), font=12)
        if note:
    ...
    def frame4(self, slide, bg, intent, arch, flow, top=1.2):
        ...
        cw, ch = (12.5 - gap_x) / 2, (5.15 - gap_y) / 2
