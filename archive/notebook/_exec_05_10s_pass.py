"""Execute 05 trial 11 and inject GIF output."""
...
def main() -> None:
    ...
    ns.update(HOLD_S10=10.0, T_LONG=12.0, AIR_12=1.00, gait_in_window=gait_in_window)
    for idx in CODE_IDX:
        ...
        gif = NB.parent / "assets/05e_inplace_trot_10s.gif"
        if "05e_inplace_trot_10s.gif" in src and gif.is_file():
            outputs.append(
                new_output(
                    "display_data",
                    data={
        ...
        nb.cells[idx].execution_count = CODE_IDX.index(idx) + 1
        if not ok:
