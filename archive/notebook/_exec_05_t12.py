"""Execute 05 trial 12 and embed GIF 05f."""
...
GIF = NB.parent / "assets/05f_inplace_trot_20s_t11.gif"
...
def main() -> None:
    ...
    for i, c in enumerate(nb.cells):
        if "trial12  trial11-ctrl" in "".join(c.source):
            idxs.append(i)
            break
    else:
        raise SystemExit("trial 12 cell not found")
    os.chdir(NB.parent)
    for idx in idxs:
        ...
        if "05f_inplace_trot_20s_t11.gif" in src and GIF.is_file():
            outputs.append(
                new_output(
                    "display_data",
                    data={
        ...
        nb.cells[idx].execution_count = idxs.index(idx) + 1
        if not ok:
