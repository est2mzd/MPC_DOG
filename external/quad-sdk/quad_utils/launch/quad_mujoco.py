...
def launch_recording(context, *args, **kwargs):
    ...
    camera_track_robot = LaunchConfiguration('camera_track_robot').perform(context).lower() == 'true'
    camera_distance = float(LaunchConfiguration('camera_distance').perform(context))
    camera_lookat_x = float(LaunchConfiguration('camera_lookat_x').perform(context))
    camera_lookat_y = float(LaunchConfiguration('camera_lookat_y').perform(context))
    ...
    return [
        Node(
            ...
            parameters=[{
                ...
                'height': 720,
                'fps': 30,
                'camera_track_robot': camera_track_robot,
...
def generate_launch_description():
    declared_args = [
        ...
        # 元は常時True(カメラがロボットのbaseへ追従)がハードコードされていた
        # (mujoco_recorder_node.cppの'camera_track_robot'パラメータ自体は元から
        # あったが、この呼び出し元では固定値渡しでlaunch引数化されていなかった)。
        # 追従カメラだと、実際に前進していても画面上は常に「その場」に見えてしまい、
        # CSVの数値だけでは前進を目視確認できないという問題があったため、
        # launch引数化してfalseを選べるようにした(MPC_DOG側の追加、
        # 既定値はtrueのままなので既存の挙動は変えない)。
        DeclareLaunchArgument('camera_track_robot', default_value='true',
            description='Whether the recorder camera follows the robot base (true) or stays fixed (false, useful to visually confirm forward locomotion)'),
        # camera_track_robot:=falseと合わせて使う想定。既定3.0mだと固定カメラの
        # 画角が狭く、ロボットがすぐ画面外に出てしまうため、任意の値を選べるようにした
        # (mujoco_recorder_node.cpp側の既存パラメータをlaunch引数化しただけ)。
        DeclareLaunchArgument('camera_distance', default_value='3.0',
            description='Recorder camera distance from lookat point (meters)'),
        # camera_track_robot:=falseのとき、固定カメラのlookat点(既定は原点付近)を
        # x/y方向にずらす。ロボットが+x方向へ歩くと画角の中心からどんどん離れて
        # いくため、進行方向側にオフセットすることで画面内に留まる時間を延ばせる
        # (mujoco_recorder_node.cpp側の既存パラメータをlaunch引数化しただけ)。
        DeclareLaunchArgument('camera_lookat_x', default_value='0.0',
            description='Fixed-camera lookat point x-offset (meters), only used when camera_track_robot=false'),
        DeclareLaunchArgument('camera_lookat_y', default_value='0.0',
            description='Fixed-camera lookat point y-offset (meters), only used when camera_track_robot=false'),
    ]
