...
RVizInterface::RVizInterface(rclcpp::Node::SharedPtr node) : node_(node) {
  ...
  // Load Parameters from Yaml File (Missing TF)
  node_->declare_parameter<std::string>("topics.global_plan", "");
  node_->declare_parameter<std::string>("topics.local_plan");
  node_->declare_parameter<std::string>("topics.control.grfs");
  node_->declare_parameter<std::string>("topics.global_plan_discrete");
  node_->declare_parameter<std::string>("topics.foot_plan_discrete");
  node_->declare_parameter<std::string>("topics.foot_plan_continuous");
  node_->declare_parameter<std::string>("topics.state.estimate");
  node_->declare_parameter<std::string>("topics.state.ground_truth");
  node_->declare_parameter<std::string>("topics.state.trajectory");
  node_->declare_parameter<std::string>("tf_prefix");

  quad_utils::loadROSParam(node_, "topics.global_plan", global_plan_topic);
  ...
  quad_utils::loadROSParamDefault(node_, "tf_prefix", tf_prefix_,
                                  std::string(""));
  ...
  node_->declare_parameter<std::string>("topics.visualization.global_plan");
  node_->declare_parameter<std::string>("topics.visualization.local_plan");
  node_->declare_parameter<std::string>("topics.visualization.local_plan_ori");
  node_->declare_parameter<std::string>("topics.visualization.global_plan_grf");
  node_->declare_parameter<std::string>("topics.visualization.local_plan_grf");
  node_->declare_parameter<std::string>("topics.visualization.current_grf");
  node_->declare_parameter<std::string>(
      "topics.visualization.global_plan_discrete");
  node_->declare_parameter<std::string>(
      "topics.visualization.foot_plan_discrete");
  node_->declare_parameter<std::string>(
      "topics.visualization.joint_states.estimate");
  node_->declare_parameter<std::string>(
      "topics.visualization.joint_states.ground_truth");
  node_->declare_parameter<std::string>(
      "topics.visualization.joint_states.trajectory");
  node_->declare_parameter<std::string>(
      "topics.visualization.state.estimate_trace");
  node_->declare_parameter<std::string>(
      "topics.visualization.state.ground_truth_trace");
  node_->declare_parameter<std::string>(
      "topics.visualization.state.trajectory_trace");

  quad_utils::loadROSParam(node_, "topics.visualization.global_plan",
                           global_plan_viz_topic);
  quad_utils::loadROSParam(node_, "topics.visualization.local_plan",
                           local_plan_viz_topic);
  quad_utils::loadROSParam(node_, "topics.visualization.local_plan_ori",
                           local_plan_ori_viz_topic);
  quad_utils::loadROSParam(node_, "topics.visualization.global_plan_grf",
                           global_plan_grf_viz_topic);
  quad_utils::loadROSParam(node_, "topics.visualization.local_plan_grf",
                           local_plan_grf_viz_topic);
  quad_utils::loadROSParam(node_, "topics.visualization.current_grf",
                           current_grf_viz_topic);
  quad_utils::loadROSParam(node_, "topics.visualization.global_plan_discrete",
                           discrete_body_plan_viz_topic);
  quad_utils::loadROSParam(node_, "topics.visualization.foot_plan_discrete",
                           foot_plan_discrete_viz_topic);
  quad_utils::loadROSParam(node_, "topics.visualization.joint_states.estimate",
  ...
  quad_utils::loadROSParam(node_, "topics.visualization.state.estimate_trace",
                           state_estimate_trace_viz_topic);
  quad_utils::loadROSParam(node_,
                           "topics.visualization.state.ground_truth_trace",
                           ground_truth_trace_viz_topic);
  quad_utils::loadROSParam(node_, "topics.visualization.state.trajectory_trace",
  ...
  node_->declare_parameter<std::vector<int>>(
      "rviz_interface.colors.front_left");
  node_->declare_parameter<std::vector<int>>("rviz_interface.colors.back_left");
  node_->declare_parameter<std::vector<int>>(
      "rviz_interface.colors.front_right");
  node_->declare_parameter<std::vector<int>>(
      "rviz_interface.colors.back_right");
  node_->declare_parameter<std::vector<int>>("rviz_interface.colors.net_grf");
  node_->declare_parameter<std::vector<int>>(
  ...
  quad_utils::loadROSParam(node_, "rviz_interface.colors.front_left",
                           front_left_color_);
  quad_utils::loadROSParam(node_, "rviz_interface.colors.back_left",
                           back_left_color_);
  quad_utils::loadROSParam(node_, "rviz_interface.colors.front_right",
                           front_right_color_);
  quad_utils::loadROSParam(node_, "rviz_interface.colors.back_right",
                           back_right_color_);
  quad_utils::loadROSParam(node_, "rviz_interface.colors.net_grf",
                           net_grf_color_);
  quad_utils::loadROSParam(node_, "rviz_interface.colors.individual_grf",
                           individual_grf_color_);

  double period, dt;
  node_->declare_parameter<double>("local_footstep_planner.period");
  node_->declare_parameter<double>("local_planner.timestep");
  quad_utils::loadROSParam(node_, "local_footstep_planner.period", period);
  quad_utils::loadROSParam(node_, "local_planner.timestep", dt);

  orientation_subsample_interval_ = int(period / dt);

  // Setup plan subs
  global_plan_sub_ = node_->create_subscription<quad_msgs::msg::RobotPlan>(
      global_plan_topic, 1,
      [this](const quad_msgs::msg::RobotPlan::SharedPtr msg) {
        this->robotPlanCallback(msg, GLOBAL);
      });

  local_plan_sub_ = node_->create_subscription<quad_msgs::msg::RobotPlan>(
      local_plan_topic, 1,
      [this](const quad_msgs::msg::RobotPlan::SharedPtr msg) {
        this->robotPlanCallback(msg, LOCAL);
      });

  grf_sub_ = node_->create_subscription<quad_msgs::msg::GRFArray>(
      grf_topic, 1,
      std::bind(&RVizInterface::grfCallback, this, std::placeholders::_1));

  foot_plan_discrete_sub_ =
      node_->create_subscription<quad_msgs::msg::MultiFootPlanDiscrete>(
          foot_plan_discrete_topic, 1,
          std::bind(&RVizInterface::footPlanDiscreteCallback, this,
                    std::placeholders::_1));

  foot_plan_continuous_sub_ =
      node_->create_subscription<quad_msgs::msg::MultiFootPlanContinuous>(
          foot_plan_continuous_topic, 1,
          std::bind(&RVizInterface::footPlanContinuousCallback, this,
                    std::placeholders::_1));

  // Setup plan visual pubs
  global_plan_viz_pub_ =
      node_->create_publisher<visualization_msgs::msg::Marker>(
          global_plan_viz_topic, 1);
  local_plan_viz_pub_ =
      node_->create_publisher<visualization_msgs::msg::Marker>(
          local_plan_viz_topic, 1);
  ...
  state_estimate_sub_ = node_->create_subscription<quad_msgs::msg::RobotState>(
      state_estimate_topic, 1,
      [this](const quad_msgs::msg::RobotState::SharedPtr msg) {
        this->robotStateCallback(msg, ESTIMATE);
      });
  ...
  ground_truth_state_sub_ =
      node_->create_subscription<quad_msgs::msg::RobotState>(
          ...
          [this](const quad_msgs::msg::RobotState::SharedPtr msg) {
            this->robotStateCallback(msg, GROUND_TRUTH);
          });
  ...
  trajectory_state_sub_ =
      node_->create_subscription<quad_msgs::msg::RobotState>(
          ...
          [this](const quad_msgs::msg::RobotState::SharedPtr msg) {
            this->robotStateCallback(msg, TRAJECTORY);
          });
  ...
  node_->declare_parameter<std::string>(
      "topics.visualization.foot_0_plan_continuous");
  node_->declare_parameter<std::string>(
      "topics.visualization.foot_1_plan_continuous");
  node_->declare_parameter<std::string>(
      "topics.visualization.foot_2_plan_continuous");
  node_->declare_parameter<std::string>(
      "topics.visualization.foot_3_plan_continuous");

  quad_utils::loadROSParam(node_, "topics.visualization.foot_0_plan_continuous",
                           foot_0_plan_continuous_viz_topic);
  quad_utils::loadROSParam(node_, "topics.visualization.foot_1_plan_continuous",
                           foot_1_plan_continuous_viz_topic);
  quad_utils::loadROSParam(node_, "topics.visualization.foot_2_plan_continuous",
                           foot_2_plan_continuous_viz_topic);
  quad_utils::loadROSParam(node_, "topics.visualization.foot_3_plan_continuous",
                           foot_3_plan_continuous_viz_topic);
  ...
  estimate_base_tf_br_ = std::make_shared<tf2_ros::TransformBroadcaster>(node_);
  ...
  state_estimate_trace_msg_.action = visualization_msgs::msg::Marker::ADD;
  state_estimate_trace_msg_.pose.orientation.w = 1;
  state_estimate_trace_msg_.type = visualization_msgs::msg::Marker::LINE_STRIP;
...
void RVizInterface::robotPlanCallback(
    const quad_msgs::msg::RobotPlan::SharedPtr msg, const int pub_id) {
  // Initialize Path message to visualize body plan
  visualization_msgs::msg::Marker body_plan_viz;
  body_plan_viz.header = msg->header;
  body_plan_viz.action = visualization_msgs::msg::Marker::ADD;
  body_plan_viz.pose.orientation.w = 1;
  body_plan_viz.id = 5;
  body_plan_viz.type = visualization_msgs::msg::Marker::LINE_STRIP;
  body_plan_viz.scale.x = 0.03;

  // Construct MarkerArray for body plan orientation
  geometry_msgs::msg::PoseArray body_plan_ori_viz;
    ...
    if (pub_id == LOCAL) {
      ...
      if (msg->primitive_ids[i] == FLIGHT) {
        ...
        RCLCPP_WARN_THROTTLE(node_->get_logger(), *node_->get_clock(), 1000,
                             "Invalid primitive ID received in RViz interface");
      }
    }
  ...
  visualization_msgs::msg::MarkerArray grfs_viz_msg;
  ...
  for (int i = 0; i < length; i++) {
    for (int j = 0; j < msg->grfs[i].vectors.size(); j++) {
      ...
      geometry_msgs::msg::Point p_base, p_tip;
...
void RVizInterface::grfCallback(const quad_msgs::msg::GRFArray::SharedPtr msg) {
...
void RVizInterface::footPlanContinuousCallback(
  ...
  std::vector<nav_msgs::msg::Path> foot_paths;
  foot_paths.resize(4);
  ...
  for (int j = 0; j < msg->states[0].feet.size(); j++) {
    ...
    for (int i = 0; i < msg->states.size(); i++) {
      geometry_msgs::msg::PoseStamped foot;
...
void RVizInterface::robotStateCallback(
    const quad_msgs::msg::RobotState::SharedPtr msg, const int pub_id) {
  ...
  if (pub_id == ESTIMATE) {
    ...
    if ((current_pos - last_pos).norm() >= trace_reset_threshold_) {
      state_estimate_trace_msg_.action =
    ...
    if ((current_pos - last_pos).norm() >= trace_reset_threshold_) {
      ...
      trajectory_state_trace_msg_.action = visualization_msgs::msg::Marker::ADD;
    }
    ...
    RCLCPP_WARN_THROTTLE(
        ...
        "Invalid publisher id, not publishing robot state to rviz");
  }
}
