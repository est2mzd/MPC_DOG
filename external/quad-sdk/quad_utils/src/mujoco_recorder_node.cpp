...
class MujocoRecorder : public rclcpp::Node {
 public:
  MujocoRecorder() : rclcpp::Node("mujoco_recorder") {
    ...
    // camera_track_robot=falseのとき(固定カメラ)のlookat点のx/yオフセット[m]。
    // 追従しないカメラだと、ロボットの初期位置(原点付近)だけを見続けるため、
    // 前進方向に画角の余裕がない(すぐ画面外に出る)というフィードバックを受け、
    // 前進方向側にlookat点をずらせるようにした(MPC_DOG側の追加)。
    declare_parameter<double>("camera_lookat_x", 0.0);
    declare_parameter<double>("camera_lookat_y", 0.0);
    ...
    cam_.lookat[0] = get_parameter("camera_lookat_x").as_double();
    cam_.lookat[1] = get_parameter("camera_lookat_y").as_double();
    cam_.lookat[2] = 0.3;
