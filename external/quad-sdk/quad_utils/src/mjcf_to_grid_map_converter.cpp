...
MjcfToGridMapConverter::MjcfToGridMapConverter(rclcpp::Node::SharedPtr node)
    : node_(node),
...
void MjcfToGridMapConverter::subscribeToTopics() {  // UPDATED
  // mesh_sub_ =
  //     nh_.subscribe("mesh", 10, &MjcfToGridMapConverter::meshCallback, this);
  mesh_sub_ = node_->create_subscription<pcl_msgs::msg::PolygonMesh>(
      "mesh", rclcpp::QoS(10),
      std::bind(&MjcfToGridMapConverter::meshCallback, this,
                std::placeholders::_1));
}

void MjcfToGridMapConverter::advertiseTopics() {  // UPDATED
...
void MjcfToGridMapConverter::advertiseServices() {  // UPDATED
  save_grid_map_srv_ = node_->create_service<grid_map_msgs::srv::ProcessFile>(
      "save_grid_map_to_file",
      std::bind(&MjcfToGridMapConverter::saveGridMapService, this,
...
bool MjcfToGridMapConverter::saveGridMapService(  // UPDATED
...
bool MjcfToGridMapConverter::saveGridMap(
...
bool MjcfToGridMapConverter::loadMeshService(  // UPDATED
...
bool MjcfToGridMapConverter::loadMeshFromFile(
