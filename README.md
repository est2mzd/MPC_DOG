...
## Quad-SDK
 ...
 - [Step 17(実装・進行中):前方ジャンプの後脚踏切。プランナ側の土台をコード+ユニットテストで実装済み — primitive ID を単一定義元に集約(`quad_utils/primitive_ids.hpp`)、`computeContactSchedule` のデッドコードを差し替えて `PRELOAD/REAR_PUSH/FLIGHT/FRONT_LAND/SETTLE`(4脚別接触)を**到達可能**に、`global_body_planner.jump_mode`(off/auto/**force_leap**)追加、`is_jump` アクションを 5 サブフェーズへ時間分割してスタンプ、踏切の水平 GRF を乱数から**着地方向**へ。`colcon test` 112 pass、既存 trot/STAND/flight 不変。**Stage 2〜6 の MuJoCo 検証(後脚踏切・四脚実離地・穴越え・安定着地の計測)は未実施**。ブランチ `feature/jump`](./agent_reports/steps/step_17_forward_jump_rear_leg_push.md)
