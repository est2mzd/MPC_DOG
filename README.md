...
## Quad-SDK
 ...
 - [Step 17(実装・進行中):前方ジャンプ。方針を「平地・穴なし・その場ジャンプ・後脚位置で計測」に絞り、`jump_mode:=force_leap` で GBP が RRT を回さず 1 回のジャンプ経路を決定論的に publish、NMPC+ID が追従。**計測**(`flat_wide`):その場ジャンプ `step17_hop_sym2` = 胴体 +0.226 m・四脚離地 264 ms・着地後 2 s の \|roll\|,\|pitch\| < 0.003 rad・NMPC 失敗 0・転倒なし。短前方ジャンプ `step17_fwd_b` = **後脚前進 +0.386 m(≥30 cm)**・四脚離地 314 ms・着地後直立維持・NMPC 失敗 0。REAR_PUSH/FRONT_LAND を実際に効かせる(姿勢発散回避のため現状は四脚対称ホップ)と穴シナリオは残課題。`colcon test` 112 pass。ブランチ `feature/jump`](./agent_reports/steps/step_17_forward_jump_rear_leg_push.md)
