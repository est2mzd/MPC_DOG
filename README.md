...
| # | 内容 | 根拠 |
|---|---|---|
| 1 | 平地の前進歩行(起立 → 歩行 → 停止)が安定 | Step 01 |
| 2 | **幅 ≤0.30 m の穴/溝を、足を入れずに複数本連続で渡る**(回帰なし) | step03/04、Step 05(15 cm 平地/15 cm 穴 ×2〜5) |
| 3 | **幅 ≥1.0 m の穴/断崖の手前で直立停止**(`edge_clearance:=0.15` で有効化時)。胴体前方 2.5 m をプローブ → `cmd_vel:=0` で減速 | Step 05b、Step 06 |
| 4 | 脚が届かない足場の検知(`ik_reach_check:=true`、専用地形で確認。既定 OFF) | Step 07 |

**できないこと(既知の穴)**

| # | 内容 | 詳細 |
|---|---|---|
| 1 | **幅およそ 0.44〜0.60 m の穴で「渡れず・止まらず・転倒」** | `max_crossable_gap`(0.6 m)が危険帯(≈ 物理幅 0.5 m)より大きく「渡れる穴」と誤判定。かつ既定 `edge_clearance:0` では幅チェック自体が走らず、足場が穴の縁セルにスナップして落ちる。→ `max_crossable_gap` を ~0.44 に下げるか、穴チェックを生 `z` の NaN で判定(**未着手**)。Step 08 / Step 09 |
| 2 | 斜め穴・旋回中の穴 | プローブは +x 方向固定。全幅横断穴のみ対応 |
| 3 | 未観測領域と穴の区別 | 地図上どちらも `NaN`。「見えていない所」を安全と誤認しうる |
| 4 | 実センサ(LiDAR/深度)からの地図生成 | この repo に無い(MuJoCo メッシュ由来のみ) |
| 5 | 複数歩先の足場列計画・停止余裕の歩数換算 | 未実装(Step 10〜16 の対象) |
...
くわしくは下記「穴対応の現状」節と
[まとめ doc](./agent_reports/quadsdk_gap_foothold_summary.md)。
