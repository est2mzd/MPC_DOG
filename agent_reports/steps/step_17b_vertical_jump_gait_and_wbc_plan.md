...
## 0. 結論（2026-09-03・この課題はここでクローズ）

- **その場・垂直ジャンプは Stage A で成立**（G1 gait=実質STAND、G3 stand_pos_error 0.15、
  W1 NMPC roll/pitch 重み 20）：実行された 12 回すべてが直立着地・転倒 0・NMPC 失敗 0。
  ホップ 0.20〜0.25 m、四脚離地 238〜290 ms、着地後 2 s の \|roll\|,\|pitch\| は 11/12 で
  < 0.1 rad。**ユーザー目標「垂直に跳んでこけずに着地」を満たしたので、ここを成果として
  クローズする**（ユーザー判断、2026-09-03）。GIF/グラフは README「Step 17」節。
- Stage B の naive W2（速度不連続を `dz_0` clamp で潰す）は**逆効果で撤回**（§9.2 W2 実測）。
  `jump_crouch_vz` パラメータは既定 0（無効）で残置。
- **Stage C/D と「正しい W2（本物のしゃがみ区間）」は未実施**。これは「余裕を増やす」
  ための追加作業であり、目標達成の可否は変えない。将来やる場合の設計は §9 に記載。
  未達の項目：飛翔中ピッチが一時 ~0.3 rad まで振れる、ホップ高さ run 間 ±8%、
  後脚のみ踏切（`REAR_PUSH`）・前脚のみ着地（`FRONT_LAND`）、穴シナリオ。
