...
- **Phase 3 完了(コード変更あり、既定 OFF)。** 足場選択後に
  ...
  **成立**(穴の約 0.7 m 手前で停止・直立保持・転落なし)。テスト **33/33**。
  詳細は下記「Phase 3」。
- **Step 05 実施済み(2026-09-01)。** Phase 2A + Phase 3(A)(`edge_clearance:=0.15`)
  で **go2 は 15 cm 平地 / 15 cm 穴の連続区間を N=2〜5 で安定して渡り切った**
  (各条件 1〜4 回、全通過、胴体 z≈0.31 保持、穴落ち・恒常的な詰まり無し)。
...
**関連 Step**:Step 05(15cm 連続穴を通過)/ Step 05b(単独断崖の安全停止 = 成立)
/ Step 06(15cm 穴 ×2 → 1m 穴:Phase 2A の丸ごと停止では手前の穴で転倒、Phase 2B 要)。
