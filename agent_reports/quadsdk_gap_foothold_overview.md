...
- **Phase 0 / 1 / 2A / 3(A) 実装済み**(下の対応表)。
- **Step 05(15 cm 連続穴 N=2〜5)は通過できた。** 事前調査の「地図 1 セル・
  幾何学的に成立困難寄り」は実測で覆った。
- **Step 05b(単独の断崖:幅 10 m / 100 cm)は手前で安全停止できた。**
- **Step 06(15 cm 穴 ×2 → 1 m 穴)は「落ちずに止まる」ができない。**
  Phase 3(A) の「渡れない穴」判定は正しく働くが、Phase 2A の
  「無効足場を 1 個検知したら local plan を丸ごと止める」が遠方ホライズンで
  発火し、手前の 15 cm 穴を渡る遊脚中に凍結 → 転倒。
  → **次は Phase 2B(能動的な停止シーケンス)。** 変更計画は
  `quadsdk_gap_foothold_phase_progress.md` 末尾 / チャット提示済み。
...
| 4. 安全に止まる(stop シーケンス) | 遊脚を着地 → 新離脚を止める → 減速 → 全脚接地 → STAND 保持 | 無い(「plan を止める → 0.1 s 後に PD ホールド」だけ。遊脚中に起きると転ぶ) | **Phase 2B(未着手、次)** |
...
| **2B** | 段 4:能動的な停止シーケンス(latch → cmd_vel:=0 → STEP→STAND → 保持) | ラッチ時のみ。plan を凍結せず減速停止 | Step 06(手前の 15 cm 穴で転ばず 1 m 穴の手前で停止)+ 全回帰 | ⬜ **次** |
| 4 | 段 2:逆運動学の可到達性で候補を絞る | ― | ― | ⬜ |
| 5 | 大きな足場補正時の減速/刻み歩行 | ― | ― | ⬜ |
| 6 | 地図の鮮度・未観測セルの扱い | ― | ― | ⬜ |
...
| **Step 06** | 15 cm 穴 ×2 → 15 cm 平地 → **1 m 穴** | `edge_clearance:0.15` | **不成立**(7 回中 直立静止 0。5 回 手前の 15 cm 穴で転倒) | 3(A) の判定は正、2A の丸ごと停止が転倒原因 → **2B 要** |
